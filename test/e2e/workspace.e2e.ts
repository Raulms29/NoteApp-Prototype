...
describe('Workspace Testing', () => {
    ...
    it('Should properly delete a Workspace', async () => {
        await POWorkspace.createWorkspace('workspace', 'Mock Workspace-3', 2);
        await POWorkspace.removeWorkspace('Mock Workspace-3');
    });

    it('Should properly select a Workspace', async () => {
        await POWorkspace.createWorkspace('workspace', 'Mock Workspace-4', 3);
    ...
    it('Should give an error when creating a Workspace with a duplicate path', async () => {
        await POWorkspace.createWorkspace('workspace', 'Mock Workspace-5', 4);
        await POWorkspace.createWorkspaceExpectError('workspace', 'Mock Workspace-5', 4);
    });

    it('Should change workspace appropiately', async () => {
        await POWorkspace.createWorkspace('workspace', 'Mock Workspace-6', 5);
        await POWorkspace.createWorkspace('workspace', 'Mock Workspace-7', 6);
